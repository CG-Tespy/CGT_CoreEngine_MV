...
export class Event 
{
    protected argCount: number;
    protected callbacks = new Map;
    protected invocationStr: string;
    protected funcToCall: Function;
    ...
    protected CheckIfArgCountIsValid(argCount: number) 
    {
        if (argCount < 0) {
    ...
    AddListener(func: Function, caller = null) 
    {
    ...
    RemoveListener(func: Function, caller = null) 
    {
        if (this.callbacks.get(caller) == null)
            return;
    ...
    protected SetupCallbackInvocationString() 
    {
