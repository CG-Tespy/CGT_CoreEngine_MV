...
export class Event
{
    protected callbacks: Map<any, Array<Function>> = new Map<any, Array<Function>>();

    protected invocationStr = '';
    protected funcToCall: Function = null;
    protected callerName = 'caller';

    // Getters
    get ArgCount()                      { return this.argCount; }

    /** Throws an exception if a negative number of args are passed. */
    constructor(protected argCount: number = 0)
    {
        this.CheckIfArgCountISValid(argCount);
        this.SetupCallbackInvocationString();
    }

    CheckIfArgCountISValid(argCount: number): void
    {
        if (argCount < 0)
        {
    ...
    AddListener(func: Function, caller: any = null): void
    {
    ...
    RemoveListener(func: Function, caller: any = null): void
    {
        if (this.callbacks.get(caller) == null)
            return;
